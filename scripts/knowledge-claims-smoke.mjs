 ...
 * → Claim, EvidenceItem and their link — against the local database.
 *
 * Normal mode calls no provider and costs nothing. `--live` routes the same
 * passage through the Model Gateway using keys already in `.env.local`
 * (values never printed); with no key present it reports BLOCKED rather
 * than weakening anything.
 *
 *   pnpm knowledge:claims:smoke
 *   pnpm knowledge:claims:smoke -- --live
 */
/**
...
const entry = resolve(root, "apps", "q-api", "dist", "dev", "claims-smoke.js");
if (!existsSync(entry)) {
